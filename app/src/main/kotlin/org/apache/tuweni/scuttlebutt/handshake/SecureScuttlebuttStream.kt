...
internal class SecureScuttlebuttStream(
    ...
    private fun decrypt(
        ...
        while (index < messageWithBuffer.size()) {
            val decryptedMessage =
                decryptMessage(messageWithBuffer.slice(index), key, nonce) ?: break
    ...
    private fun decryptMessage(message: Bytes, key: SecretBox.Key, nonce: MutableBytes): Bytes? {
        ...
        return try {
            ...
            val bodySize =
                (decryptedHeader[0].toInt() and 0xFF shl 8) + (decryptedHeader[1].toInt() and 0xFF)
    ...
    private fun encrypt(
        message: Bytes,
        clientToServerKey: SecretBox.Key,
        clientToServerNonce: MutableBytes
    ): Bytes {
    ...
    private fun encryptMessage(message: Bytes, key: SecretBox.Key, nonce: MutableBytes): Bytes {
        ...
        return try {
            ...
            val header = SecretBox.encrypt(
                Bytes.concatenate(encodedBodySize, encryptedBody.slice(0, 16)),
                key,
                headerNonce
            )
