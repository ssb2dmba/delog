...
object RPCCodec {
    ...
    fun encodeRequest(body: String, vararg flags: RPCFlag): Bytes {
        return encodeRequest(
            Bytes.wrap(body.toByteArray(StandardCharsets.UTF_8)),
            nextRequestNumber(),
            *flags
        )
    }
    ...
    fun encodeRequest(body: Bytes, requestNumber: Int, vararg flags: RPCFlag): Bytes {
        val encodedRequestNumber = if (requestNumber >= 1)
            Bytes.ofUnsignedInt(requestNumber.toLong())
        else Bytes.wrap(ByteBuffer.allocate(4).putInt(requestNumber).array())
