...
class SecureScuttlebuttHandshakeClient private constructor(
    ...
    fun createHello(): Bytes {
        val hmac =
            HMACSHA512256.authenticate(ephemeralKeyPair.publicKey().bytes(), networkIdentifier)
    ...
    fun createIdentityMessage(): Bytes {
        ...
        detachedSignature =
            Signature.signDetached(concatenate.concatenate(), longTermKeyPair.secretKey())
        return SecretBox
            .encrypt(
                Concatenate().add(detachedSignature!!).add(longTermKeyPair.publicKey())
                    .concatenate(),
                SecretBox.Key
                    .fromHash(
                        SHA256Hash
    ...
    fun clientToServerNonce(): Bytes {
        return HMACSHA512256.authenticate(serverEphemeralPublicKey!!.bytes(), networkIdentifier)
            .slice(0, 24)
    }
    ...
    fun serverToClientNonce(): Bytes {
        return HMACSHA512256.authenticate(ephemeralKeyPair.publicKey().bytes(), networkIdentifier)
            .slice(0, 24)
    }
