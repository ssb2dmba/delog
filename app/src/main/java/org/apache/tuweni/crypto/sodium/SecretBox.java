...
public final class SecretBox {
    ...
    public static byte[] encrypt(byte[] message, Key key, Nonce nonce) {
        ...
        int rc =
                Sodium.crypto_secretbox_easy(cipherText, message, message.length, nonce.value.bytesArray(), key.value.bytesArray());
        if (rc != 0) {
            throw new SodiumException("crypto_secretbox_easy: failed with result " + rc);
        }
    ...
    public static byte[] encrypt(
        ...
        try {
            rc = Sodium
                    .crypto_secretbox_easy(cipherText, message, message.length, nonce.value.bytesArray(), key.value.bytesArray());
        } finally {
            key = null;
        }
        if (rc != 0) {
    ...
    private static byte[] prependNonce(Nonce nonce, byte[] bytes) {
        ...
        System.arraycopy(nonce.value.bytesArray(), 0, data, 0, nonceLength);
...
///            Pointer ptr = Sodium.malloc(length);
    ...
    // prepend the nonce to the cyphertext ?
    ...
    public static final class Nonce implements Destroyable {
        ...
        public static Nonce fromBytes(byte[] bytes) {
            ...
            return SodiumUtils.dup(bytes, Nonce::new);
        }
        ...
        public Nonce increment() {
            return SodiumUtils.dupAndIncrement(value.bytesArray(), length(), Nonce::new);
        }
