...
public final class Signature {
    ...
    public static Allocated signDetached(Allocated message, SecretKey secretKey) {
        ...
        int rc = Sodium
                .crypto_sign_detached(
                        signature.bytesArray(),
                        new int[]{Sodium.crypto_sign_bytes()},
                        message.bytesArray(),
                        (int) message.length(),
                        secretKey.value.bytesArray());
        if (rc != 0) {
    ...
    public static byte[] signDetached(byte[] message, SecretKey secretKey) {
        ...
        int[] sig_len = new int[1];
        int rc = Sodium.crypto_sign_detached(signature, sig_len, message, message.length, secretKey.value.bytesArray());
    ...
    public static boolean verifyDetached(byte[] message, byte[] signature, PublicKey publicKey) {
        int rc = Sodium.crypto_sign_verify_detached(signature, message, message.length, publicKey.value.bytesArray());
        if (rc == -1) {
            return false;
        }
        if (rc != 0) {
    ...
    public static byte[] sign(byte[] message, SecretKey secretKey) {
        ...
        int rc = Sodium.crypto_sign(signature, sign_len, message, message.length, secretKey.value.bytesArray());
        if (rc != 0) {
    ...
    public static byte[] verify(byte[] signed, PublicKey publicKey) {
        ...
        int[] messageLongReference = new int[1];
        int rc = Sodium.crypto_sign_open(message, messageLongReference, signed, signed.length, publicKey.value.bytesArray());
        if (rc != 0) {
            throw new SodiumException("crypto_sign_open: failed with result " + rc);
        }

        return Arrays.copyOfRange(message, 0, messageLongReference[0]);
    }
    ...
    public static final class PublicKey implements Destroyable {
        ...
        public static Signature.PublicKey fromBytes(byte[] bytes) {
            ...
            return SodiumUtils.dup(bytes, PublicKey::new);
        }
        ...
        public boolean equals(Object obj) {
            ...
            return Objects.equals(this.value.bytes().toHexString(), other.value.bytes().toHexString());
        }
    ...
    public static final class SecretKey implements Destroyable {
        ...
        public static SecretKey fromBytes(byte[] bytes) {
            ...
            return SodiumUtils.dup(bytes, SecretKey::new);
        }

        public static SecretKey fromSeed(Seed seed) {
            return SodiumUtils.dup(seed.bytes().toArray(), SecretKey::new);
        }
    ...
    public static final class Seed {
        ...
        public static Seed fromBytes(byte[] bytes) {
            ...
            return SodiumUtils.dup(bytes, Seed::new);
        }
        ...
        public static Seed random() {
            return SodiumUtils.randomBytes(length(), Seed::new);
        }
...
//      Pointer publicKey = Sodium.malloc(publicKeyLength);
            ...
            try {
                int rc = Sodium.crypto_sign_ed25519_sk_to_pk(publicKey, secretKey.value.bytesArray());
                if (rc != 0) {
        ...
        public static KeyPair fromSeed(Seed seed) {
            ...
            try {
                int secretKeyLength = SecretKey.length();
                secretKey = new byte[secretKeyLength];
                int rc = Sodium.crypto_sign_seed_keypair(publicKey, secretKey, seed.value.bytesArray());
                if (rc != 0) {
