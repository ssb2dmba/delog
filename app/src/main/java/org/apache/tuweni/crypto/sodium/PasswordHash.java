...
public final class PasswordHash {
    ...
    public static byte[] hash(byte[] password, int length, Salt salt, long opsLimit, long memLimit, Algorithm algorithm) {
        ...
        int rc = Sodium.crypto_pwhash(out, length, password, password.length, salt.value.bytesArray(), (int) opsLimit, (int) memLimit, algorithm.id);
        if (rc != 0) {
    ...
    private static void assertHashLength(int length) {
        if (length < Sodium.crypto_pwhash_bytes_min()) {
    ...
    public static String hash(String password, long opsLimit, long memLimit) {
        ...
        int rc = Sodium.crypto_pwhash_str(out, pwBytes, pwBytes.length, (int) opsLimit, (int) memLimit);
        if (rc != 0) {
    ...
    public static boolean verify(String hash, String password) {
        ...
        byte[] str = new byte[hashLength]; // Sodium.malloc(hashLength);
        try {
            System.arraycopy(hash.getBytes(UTF_8).clone(), 0, str, 0, hash.getBytes(UTF_8).length);
    ...
    public static VerificationResult checkHash(String hash, String password, long opsLimit, long memLimit) {
        ...
        byte[] str = new byte[hashLength + 1];
        try {
            byte[] pwBytes = password.getBytes(UTF_8);
            ...
            int rc = Sodium.crypto_pwhash_str_needs_rehash(str, (int) opsLimit, (int) memLimit);
            if (rc < 0) {
    ...
    public static boolean needsRehash(String hash, long opsLimit, long memLimit) {
        ...
        byte[] str = new byte[hashLength + 1];
        System.arraycopy(hashBytes, 0, str, 0, hashBytes.length);
        int rc = Sodium.crypto_pwhash_str_needs_rehash(str, (int) opsLimit, (int) memLimit);
        if (rc < 0) {
            throw new SodiumException("crypto_pwhash_str_needs_rehash: failed with result " + rc);
        }
    ...
    private static void assertOpsLimit(long opsLimit) {
        if (opsLimit < minOpsLimit() || (maxOpsLimit() > 0 && opsLimit > maxOpsLimit())) {
            throw new IllegalArgumentException("opsLimit out of range with: " + opsLimit + " " + maxOpsLimit());
        }
    }
    ...
    private static void assertMemLimit(long memLimit) {
        if (memLimit < minMemLimit() || (maxMemLimit() > 0 && memLimit > maxMemLimit())) {
            throw new IllegalArgumentException("memLimit out of range with: " + memLimit);
        }
    }
    ...
    public static final class Salt {
        ...
        private Salt(byte[] ptr, int length) {
        ...
        public static Salt fromBytes(byte[] bytes) {
            ...
            return SodiumUtils.dup(bytes, Salt::new);
        }
        ...
        public static Salt random() {
            return SodiumUtils.randomBytes(length(), Salt::new);
        }
    ...
    public static final class Algorithm {
        ...
        private static Algorithm ARGON2ID13 = new Algorithm("argon2id13", 2, 1, true);
