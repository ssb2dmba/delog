...
public final class Box implements AutoCloseable {
    ...
    private Box(PublicKey publicKey, SecretKey secretKey) {
        ...
        ctx = new byte[Sodium.crypto_box_beforenmbytes()];
        try {
            int rc = Sodium.crypto_box_beforenm(ctx, publicKey.value.bytesArray(), secretKey.value.bytesArray());
            if (rc != 0) {
    ...
    public static byte[] encrypt(byte[] message, PublicKey receiver, SecretKey sender, Nonce nonce) {
        ...
        int rc = Sodium
                .crypto_box_easy(
                        ...
                        message,
                        message.length,
                        nonce.value.bytesArray(),
                        receiver.value.bytesArray(),
                        sender.value.bytesArray());
        if (rc != 0) {
    ...
    public static byte[] encryptSealed(byte[] message, PublicKey receiver) {
        ...
        int rc = Sodium.crypto_box_seal(cipherText, message, message.length, receiver.value.bytesArray());
        if (rc != 0) {
    ...
    public byte[] encrypt(byte[] message, Nonce nonce) {
        ...
        int rc = Sodium.crypto_box_easy_afternm(cipherText, message, message.length, nonce.value.bytesArray(), ctx);
        if (rc != 0) {
    ...
    public DetachedEncryptionResult encryptDetached(byte[] message, Nonce nonce) {
        ...
        int rc = Sodium.crypto_box_detached_afternm(cipherText, mac, message, message.length, nonce.value.bytesArray(), ctx);
        if (rc != 0) {
    ...
    public byte[] decrypt(byte[] cipherText, Nonce nonce) {
        ...
        int rc = Sodium.crypto_box_open_easy_afternm(clearText, cipherText, cipherText.length, nonce.value.bytesArray(), ctx);
        if (rc == -1) {
            return null;
        }
        if (rc != 0) {
    ...
    public byte[] decryptDetached(byte[] cipherText, byte[] mac, Nonce nonce) {
        ...
        byte[] clearText = new byte[cipherText.length];
        int rc = Sodium
                .crypto_box_open_detached_afternm(clearText, cipherText, mac, cipherText.length, nonce.value.bytesArray(), ctx);
        if (rc == -1) {
            return null;
        }
        if (rc != 0) {
    ...
    public static final class PublicKey implements Destroyable {
        ...
        private PublicKey(byte[] ptr, int length) {
        ...
        public static PublicKey fromBytes(byte[] bytes) {
            ...
            return new PublicKey(bytes, bytes.length);
        }
        ...
        public static PublicKey forSignaturePublicKey(Signature.PublicKey publicKey) {
            ...
            byte[] publicKeyPtr = null;
            try {
                int publicKeyLength = PublicKey.length();
                publicKeyPtr = new byte[publicKeyLength];
                int rc = Sodium.crypto_sign_ed25519_pk_to_curve25519(publicKeyPtr, publicKey.value.bytesArray());
                if (rc != 0) {
        ...
        public boolean equals(Object obj) {
            ...
            return this.value.bytes().toHexString().equals(other.value.bytes().toHexString());
        }
    ...
    public static final class SecretKey {
        ...
        private SecretKey(byte[] ptr, int length) {
            this.value = new Allocated(ptr, length);
        }
        ...
        public static SecretKey fromBytes(byte[] bytes) {
            ...
            return new SecretKey(bytes.clone(), bytes.length);
        }
        ...
        public static SecretKey forSignatureSecretKey(Signature.SecretKey secretKey) {
            ...
            byte[] secretKeyPtr = null;
            try {
                int secretKeyLength = SecretKey.length();
                secretKeyPtr = new byte[secretKeyLength];
                int rc = Sodium.crypto_sign_ed25519_sk_to_curve25519(secretKeyPtr, secretKey.value.bytesArray());
                if (rc != 0) {
        ...
        public boolean equals(Object obj) {
            ...
            return other.value.bytes().toHexString().equals(this.value.bytes().toHexString());
        }
    ...
    public static final class Seed {
        ...
        private Seed(byte[] ptr, int length) {
            this.value = new Allocated(ptr, length);
        }
        ...
        public static Seed fromBytes(byte[] bytes) {
            ...
            return new Seed(bytes.clone(), bytes.length);
        }
    ...
    public static final class KeyPair {
        ...
        public static KeyPair forSecretKey(SecretKey secretKey) {
            return SodiumUtils.scalarMultBase(secretKey.value.bytesArray(), SecretKey.length(), (ptr, len) -> {
                int publicKeyLength = PublicKey.length();
        ...
        public static KeyPair random() {
            int publicKeyLength = PublicKey.length();
            byte[] publicKey = new byte[publicKeyLength]; // Sodium.malloc(publicKeyLength);
            byte[] secretKey = null;
            try {
                int secretKeyLength = SecretKey.length();
        ...
        public static KeyPair fromSeed(Seed seed) {
            ...
            try {
                int secretKeyLength = SecretKey.length();
                secretKey = new byte[secretKeyLength];
                int rc = Sodium.crypto_box_seed_keypair(publicKey, secretKey, seed.value.bytesArray());
                if (rc != 0) {
                    throw new SodiumException("crypto_box_keypair: failed with result " + rc);
                }
                PublicKey pk = new PublicKey(publicKey, publicKeyLength);
    ...
    public static final class Nonce {
        ...
        private Nonce(byte[] ptr, int length) {
            this.value = new Allocated(ptr, length);
        }
        ...
        public static Nonce fromBytes(byte[] bytes) {
            ...
            return new Nonce(bytes.clone(), bytes.length);
        }
        ...
        public static Nonce zero() {
            int length = length();
            return new Nonce(new byte[length], length);
        }
        ...
        public static Nonce random() {
            byte[] r = new byte[length()];
            Sodium.randombytes(r, r.length);
            return new Nonce(r, r.length);
        }
        ...
        public Nonce increment() {
            Sodium.sodium_increment(value.bytesArray(), length());
            return SodiumUtils.dupAndIncrement(value.bytesArray(), value.length(), Nonce::new);
        }
